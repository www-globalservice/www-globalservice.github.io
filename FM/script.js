...
const AppState = {
    ...
    data: {               // Estructura BD
        products: [],
        invoices: [],
        config: { lastInvoiceNumber: 1, defaultTaxRate: 12, boxNumber: "01", lastUpdated: "" }
    },
    cart: [],
...
const API = {
    ...
    async publishData() {
        ...
        if (AppState.publishHistory.length >= CONFIG.PUBLISH_MAX_CALLS) {
            Utils.showToast(`Límite alcanzado. Intenta en 1 minuto.`, "warning");
            return false;
        }
        ...
        try {
            ...
            Storage.saveLocal(); // Guardar el nuevo timestamp localmente

            // 2. Preparar payload (PUT)
...
const Storage = {
    ...
    async initDataFlow() {
        ...
        if (hasLocal) {
            this.loadLocal();
            // Verificar remoto silenciosamente para ver si hay conflicto de fechas (Opcional)
            // Por requerimiento, preguntamos si usar local o remoto.
            document.getElementById('modal-sync').classList.add('active');
        } else {
            // No hay local, forzar remoto
...
const Business = {
    ...
    generateInvoiceRecord(client, rif, taxRate) {
        ...
        const now = new Date();
        const invoiceNum = Utils.generatePad(AppState.data.config.lastInvoiceNumber, 8);
        ...
        AppState.data.invoices.unshift(record); // Guardar al inicio
    ...
    saveProduct(prodObj) {
        ...
        if (index > -1) {
            // Actualizar, preservando el estado suspendido si no se envía explícitamente
...
const UI = {
    ...
    handleSaveProduct(e) {
        ...
        // Validación de ID único si es nuevo o cambió el ID
        ...
        // Si cambió el ID de un producto existente, borrar el viejo
        if (origId && origId !== newId) {
            const oldProd = AppState.data.products.find(p => p.id === origId);
            if (oldProd) prod.suspended = oldProd.suspended; // heredar estado
    ...
    updateSystemStatus() {
        document.getElementById('sys-last-update').innerText = AppState.data.config.lastUpdated 
            ...
            : 'Nunca';
        document.getElementById('sys-next-ticket').innerText = Utils.generatePad(AppState.data.config.lastInvoiceNumber, 8);
    ...
    editProduct(id) {
        ...
        document.getElementById('form-prod-id').disabled = true; // No permitir cambiar ID en edición para evitar inconsistencias graves, o permitirlo bajo propio riesgo
    ...
    fillPrintTicket(inv, isReprint) {
        ...
        document.getElementById('t-box-num').innerText = AppState.data.config.boxNumber || "01";
        ...
        // Barcode
        JsBarcode(document.getElementById('barcode'), inv.numero, { format:"CODE128", width:2, height:35, displayValue:false, margin:0 });
    },
